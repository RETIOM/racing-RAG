...
# TO BE MODIFIED ACCORDING TO DATABASE TREE STRUCTURE
def traverse_tree(tree, query: list[float], k: int) -> list[Document]:
    documents = []
    s_current = tree[0] # first layer of tree/temporary
    for layer in range(4):   # 4 is temporary, arbitrary; replace with number of layers
        ...
        documents.append(sorted(top_k, key=lambda x: x[1], reverse=True)[:k])
        s_current = layer
